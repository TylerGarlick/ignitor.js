...
function Model(values, opts) {
  ...
  var Repository = new require('./index');
  ...
  if (!Model._meta.repository) {
    var repository;
    if (this.opts.isMock) {
      repository = new Repository.Mock(Model._meta.collection, Model._meta.attributes);
    } else {
      repository = new Repository.Arango(Model._meta.collection, Model._meta.attributes);
    }
